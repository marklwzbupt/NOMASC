...
class AveragedRangeTracker(RangeTracker):  
    ...
    def update_range(self, min_val, max_val):
        ...
        self.min_val = torch.tensor([-4]).cuda()
        self.max_val = torch.tensor([6]).cuda()
...
class UnsignedQuantizer(Quantizer):
    def __init__(self, *args, **kwargs):
        ...
        # unsigned量化器，最大值为(2^bits - 1), 最小值为0
...
class AsymmetricQuantizer(UnsignedQuantizer):
    ...
    def update_params(self):
        ...
        # zero_point = -4 * 0.3 = -1.2
        self.zero_point = torch.round(self.range_tracker.min_val * self.scale)
        #self.zero_point = torch.round(self.range_tracker.min_val * self.scale) - 0.5
        # print(self.scale) # 0.3
        # print(self.zero_point) # -2
class SignedQuantizer(Quantizer):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # the range of sign number is (-2^(bits - 1), 2^(bits - 1) - 1 )
        self.register_buffer('min_val', torch.tensor(-(1 << (self.bits - 1))))    # -2^(bits - 1) # -2
