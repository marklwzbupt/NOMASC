...
% 调制
n_mod = qammod(n_ldpc, M, 'InputType', 'bit', 'UnitAveragePower', true);
f_mod = qammod(f_ldpc, M, 'InputType', 'bit', 'UnitAveragePower', true);
% 叠加编码
...
f_demod = qamdemod(y_f / sqrt(f_rou),M,'OutputType','llr', 'UnitAveragePower', true, 'NoiseVariance', (var1+n_rou) / f_rou);
f_bit_esti = ldpcDecoder(f_demod);

%mean(sc_coding .* conj(sc_coding))
% n用户sic解码
var2 = mean(sc_coding .* conj(sc_coding)) / (10 ^ (n_snr / 10));
f_n_demod = qamdemod(y_n / sqrt(f_rou), M, 'OutputType','llr', 'UnitAveragePower', true, 'NoiseVariance', (var2+n_rou) / f_rou );
f_n_bit = ldpcDecoder(f_n_demod);

f_n_bit_ldpc = ldpcEncoder(double(f_n_bit));
f_n_bit_remod = qammod(f_n_bit_ldpc, M, 'InputType', 'bit', 'UnitAveragePower', true);
...
n_demod = qamdemod(residual./ sqrt(n_rou), M, 'OutputType','llr','UnitAveragePower', true,  'NoiseVariance', var2 ./ n_rou);
n_bit_esti = ldpcDecoder(n_demod);
